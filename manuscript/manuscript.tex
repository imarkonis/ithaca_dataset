...
Despite their importance and the best efforts of the scientific community, terrestrial precipitation and evapotranspiration remain shrouded in uncertainty due to their intrinsic complexity as multiscale physical processes and practical limitations \citep{su_precipitation_2026}. Precipitation datasets are derived from manifold sources, including in situ gauges, satellites, reanalyses, and, most recently, merged products, each with distinct strengths and limitations related to spatial coverage, retrieval physics, temporal sampling, gauge undercatch artifacts, and orographic effects \citep{sun_review_2018}, which translate into unavoidable yet discordant bias-correction procedures. Evapotranspiration datasets, while similarly heterogeneous, are further complicated by the practical complexity of direct observations and the intertwining of evaporation and transpiration processes. The scarcity of in-situ evapotranspiration measurements forced us to rely on potential evapotranspiration-derived formulations, leading to substantial divergence among products in magnitude, variability, and long-term trends \citep{thakur_unveiling_2025}. This issue is particularly dire for $P−E$, since it inherits uncertainty from both component fluxes and may amplify inconsistencies when precipitation and evapotranspiration are combined from independently developed products. As a result, when working across different regions, conclusions about terrestrial water availability, changes in the water cycle, or hydrological partitioning can depend strongly on the dataset selected, even when analyses are otherwise identical.

In an effort to overcome individual dataset shortcomings, an ensemble and multisource thinking style approach has been widely adopted by the scientific community, as reflected in the increasing number of studies comparing multiple products or averaging them to compute more stable fields \citep{abbas_comprehensive_2026, thomson_agreement_2026}. While most practical applications handle uncertainty informally and rely on product spread to quantify an uncertainty range, this approach rarely indicates whether multiplicity artifacts exist among the selected datasets or whether some products are more plausible than others in a given region, land cover type, or hydroclimatic regime \citep{vargas_godoy_which_2025}. It has been previously reported that dataset performance is domain-dependent, with non-negligible differences among products across humid and arid regions, mountainous terrain, forested landscapes, and observation-rich versus observation-poor areas \citep{markonis_spatial_2024}. Therefore, uncertainty should not be represented as a single global spread statistic, but as a geographically distributed property of the dataset itself.

Herein, we aim to introduce a dataset designed within a framework that acknowledges how multiple $P–E$ realizations may be plausible and that their probabilities should vary by region and biome, depending on the behavior of the contributing products and local hydroclimatic conditions. Thus, it permits sampling probable alternatives rather than inheriting a deterministic estimate whose local reliability is unclear. The dataset is a probabilistic representation of precipitation and evapotranspiration with global land coverage for the 1982$-$2021 period, built so that no single input dataset is privileged. To achieve this, the dataset choice is determined by Monte Carlo sampling from performance-based probabilities rather than a single ``best-estimate'' merge. Furthermore, sampling is stratified by IPCC AR6 region and biome, so spatial coherence of dataset selection is preserved within physically meaningful units rather than drawn independently per grid cell. In this sense, the dataset is not another merged product, but a probabilistic description that offers globally consistent precipitation, evapotranspiration, and $P−E$ fields with their respective uncertainties. Consequently, enabling more transparent data reuse and supporting a shift from deterministic dataset selection toward reproducible ensemble-based use of terrestrial water cycle estimates, which is increasingly needed given the diversity and interdependence of modern Earth system data products.
